...
func create(w http.ResponseWriter, r *http.Request){
	...
	c := session.DB("dojogo").C("people")
	err = c.Insert(&Person{usuario.Name, usuario.Phone, usuario.Id, usuario.Email, usuario.Address})
	if err != nil {
					log.Fatal(err)
	}

	result := Person{}
	/*err = c.Find(bson.M{"name": "juan"}).One(&result)
	if err != nil {
					log.Fatal(err)
	}*/
...
func getByEmail(w http.ResponseWriter, r *http.Request){
	...
	var usuarios []Person
	c := session.DB("dojogo").C("people")
	err = c.Find(bson.M{"Email": email}).All(&usuarios)
	respuesta, err :=  json.Marshal(usuarios)
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
	w.Write(respuesta)
...
func main(){
	...
	r.HandleFunc("/userEmail/{emailUser}", getByEmail).Methods("GET")
